...
function AdminPanel() {
    ...
    const addItem = async () => {
        await supabase.from('scp_subjects').insert([newRecord]);
        setNewRecord({ id: '', class: '', image: '', description: '', containment: '' });
        window.location.reload();
    };

    const deleteItem = async (id) => {
        await supabase.from('scp_subjects').delete().eq('id', id);
        window.location.reload();
    };
    ...
    const saveEdit = async (id) => {
        await supabase.from('scp_subjects').update(editRecord).eq('id', id);
        setEditRecord(null);
        window.location.reload();
    };
